...
void a64_cbz_helper(uint32_t *write_p, uint64_t target, uint32_t sf, uint32_t rt) {
  int ret __attribute__((unused)) = a64_cbz_cbnz_helper(write_p, false, target, sf, rt);
  assert(ret == 0);
}

void a64_cbnz_helper(uint32_t *write_p, uint64_t target, uint32_t sf, uint32_t rt) {
  int ret __attribute__((unused)) = a64_cbz_cbnz_helper(write_p, true, target, sf, rt);
  assert(ret == 0);
}
...
void a64_check_free_space(dbm_thread *thread_data, uint32_t **write_p,
  ...
  while ((((uint64_t)*write_p) + size) >= (uint64_t)*data_p) {
    ...
    for (int i = 0; i < global_data.free_plugin; i++) {
      if (global_data.plugins[i].cbs[cb_id] != NULL) {
        ...
        if (allow_write) {
          ...
          assert(((count_bits(ctx.code.pushed_regs) + 1) & ~1) == ctx.code.plugin_pushed_reg_count);
